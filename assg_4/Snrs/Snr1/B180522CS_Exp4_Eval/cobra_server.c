...
void * communicate(void * Client_details)
{
	...
	if(active==1)
	{
		sprintf(buffer, "No other CO-PIs connected.\nPls wait\n");
...
int main() 
{
	...
	while(1)
	{
		...
		int pwd=234513;
		char password[10];
		int bytes = recv(socketID, password, sizeof(password), 0);
		password[bytes] = '\0';

		int client_pwd = atoi(password);

		//check password
		if(pwd != client_pwd)
		{
			char buffer[256];
			sprintf(buffer, "Wrong password, authentication failed\n");
			send(socketID, buffer, sizeof(buffer), 0);
			close(socketID);
			continue;
		}
		...
		{
			char buffer[256];
			sprintf(buffer, "Welcome to Project COBRA\n");
			send(socketID, buffer, sizeof(buffer), 0);
		}

		char name[256];
		bytes = recv(socketID, name, sizeof(name), 0);
		name[bytes] = '\0';
